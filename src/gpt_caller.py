...
class LLM:
    def _translate_en2zh(self, english_content):
    ...
    def summary_logic(self, data_list):
        result = []
        for data in data_list:
            if self._count_tokens(data) > 11000:
                data = data[:64500]
            prompt = f"""{data}\n以上是某論文內容，你要將重點觀念和內容提到的論點進行摘要，摘要700字即可，中文條列式重點摘要："""
            res = self._call_openai_api(prompt=prompt,temperature=0,max_tokens=2500)
            res = self._translate_en2zh(res)
            result.append(res)
        return result
